import {ModalController, NavController, ViewController, NavParams, Events, Slides} from 'ionic-angular';
...
export class IonicGallery {
    ...
    constructor(private nav: NavController, private elementRef: ElementRef, public modalCtrl: ModalController) {
    }
    ...
    presentImageModal(index: number) {
        let imageModal = this.modalCtrl.create(ImageView, {items: this.items, options: this.options, index: index});
        console.log("presentImageModal", index, imageModal);
        imageModal.onDidDismiss(data => {
        });
        imageModal.present(imageModal);
    }
